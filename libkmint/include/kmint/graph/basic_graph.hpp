...
public:
  ...
  */
  ...
  void untag_all() {
    std::for_each(begin(), end(), [](auto &n) { n.tagged(false); });
  }

  void tag(node_iterator beginTagged, node_iterator endTagged) {
    untag_all();
    std::for_each(beginTagged, endTagged, [](auto &n) { n.tagged(true); });
  }
